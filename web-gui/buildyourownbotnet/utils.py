...
def get_sessions_serialized(user_id):
	...
	return [session.serialize() for session in dao.get_sessions(user_id)]


def get_tasks_serialized(session_uid):
	"""Return serialized list of tasks for a given session."""
	tasks = dao.get_tasks(session_uid)
	serialized_tasks = []
	for task in tasks:
		task = task.serialize()
		if task.get('result'):
...
def get_tasks_serialized_paginated(session_id, page=1):
	...
	tasks, pages = dao.get_tasks_paginated(session_id, page=page)
	serialized_tasks = []
	for task in tasks:
		task = task.serialize()
		if task.get('result'):
